...
public:
  ...
  Error(Position pos_start, Position pos_end, string error_name, string details,
        string error_code = "E0000", string hint = "")
      : pos_start(std::move(pos_start)), pos_end(std::move(pos_end)),
        error_name(std::move(error_name)), details(std::move(details)),
        error_code(std::move(error_code)), hint(std::move(hint)) {}

  [[nodiscard]] virtual string to_string() const {
    string rel_path =
        pos_start.file_name ? get_relative_path(*pos_start.file_name) : "";
    string snippet =
        pos_start.file_text
            ? string_with_arrows(*pos_start.file_text, pos_start, pos_end)
            : "";
    string result =
        std::format("  [{}] File {}, line {}\n{}\n{}: {}", error_code,
                    rel_path.empty() ? "<unknown>" : rel_path,
                    pos_start.line + 1, snippet, error_name, details);
    if (!hint.empty()) {
      result += std::format("\n  Hint: {}", hint);
    }
    return result;
...
protected:
  ...
  static string string_with_arrows(const string &text,
    ...
    // Smart Truncation for exceptionally long lines (e.g. minified files or
    // flat expressions)
    ...
    if ((int)line.size() > MAX_LINE_LENGTH) {
      ...
      // Re-adjust start_idx if near the end of the line to show more left
      // context
      ...
      int new_col_start =
          max(0, min((int)truncated.size(), col_start + offset));
      int new_col_end =
          max(new_col_start + 1, min((int)truncated.size(), col_end + offset));

      string snippet = indent + truncated + "\n";
      snippet += indent + string(new_col_start, ' ') +
                 string(new_col_end - new_col_start, '^');
      return snippet;
    }

    string snippet = indent + line + "\n";
    snippet +=
        indent + string(col_start, ' ') + string(col_end - col_start, '^');
    return snippet;
  }
};
...
public:
  IllegalCharError(const Position &pos_start, const Position &pos_end,
                   const string &details = "", string hint = "")
      : Error(pos_start, pos_end, "Illegal Character Error", details, "E1001",
              hint.empty() ? "Remove or replace the unrecognised character."
                           : std::move(hint)) {}
...
public:
  ExpectedCharError(const Position &pos_start, const Position &pos_end,
                    const string &details = "", string hint = "")
      : Error(pos_start, pos_end, "Expected Character", details, "E1002",
              std::move(hint)) {}
...
public:
  InvalidSyntaxError(const Position &pos_start, const Position &pos_end,
                     const string &details = "", string hint = "")
      : Error(pos_start, pos_end, "Invalid Syntax Error", details, "E2001",
              std::move(hint)) {}
...
public:
  ...
  RunTimeError(const Position &pos_start, const Position &pos_end,
               const string &details, shared_ptr<Context> context,
               const string &error_name = "RunTimeError",
               string error_code = "E9001", string hint = "")
      : Error(pos_start, pos_end, error_name, details, std::move(error_code),
              std::move(hint)),
  ...
  [[nodiscard]] string generate_traceback() const {
    ...
    while (ctx && frames.size() < 1000) {
      ...
      if (!pos.file_name || pos.file_name->empty()) {
        ...
        snippet = pos.file_text
                      ? string_with_arrows(*pos.file_text, pos, end_pos)
                      : "";
        line_str = std::to_string(pos.line + 1);
      }

      frames.push_back(std::format("  File {}, line {}, in {}\n{}", rel_path,
                                   line_str, ctx->display_name, snippet));
    ...
    string result = std::format(
        ...
        traceback_body, this->error_name, this->details);
    if (!hint.empty()) {
      result += std::format("\n  Hint: {}", hint);
    }
...
public:
  IllegalOperationError(const Position &pos_start, const Position &pos_end,
                        const string &details, shared_ptr<Context> context,
                        string hint = "")
      : RunTimeError(pos_start, pos_end, details, std::move(context),
                     "IllegalOperationError", "E5001", std::move(hint)) {}
...
public:
  DivisionByZeroError(const Position &pos_start, const Position &pos_end,
                      const string &details, shared_ptr<Context> context,
                      string hint = "")
      : RunTimeError(pos_start, pos_end, details, std::move(context),
                     "DivisionByZeroError", "E5002",
                     hint.empty()
                         ? "Check that the divisor is not zero before "
                           "dividing. Example: `when b != 0 { result = a / b }`"
                         : std::move(hint)) {}
...
public:
  IndexOutOfBoundsError(const Position &pos_start, const Position &pos_end,
                        const string &details, shared_ptr<Context> context,
                        string hint = "")
      : RunTimeError(pos_start, pos_end, details, std::move(context),
                     "IndexOutOfBoundsError", "E7001",
                     hint.empty()
                         ? "Use `len(collection) - 1` to find the last valid "
                           "index, or check bounds before accessing."
                         : std::move(hint)) {}
...
public:
  NameError(const Position &pos_start, const Position &pos_end,
            const string &details, shared_ptr<Context> context,
            string hint = "")
      : RunTimeError(pos_start, pos_end, details, std::move(context),
                     "NameError", "E3001",
                     hint.empty() ? "Check for typos or make sure the variable "
                                    "is assigned before use."
                                  : std::move(hint)) {}
...
public:
  ArgumentError(const Position &pos_start, const Position &pos_end,
                const string &details, shared_ptr<Context> context,
                string hint = "")
      : RunTimeError(pos_start, pos_end, details, std::move(context),
                     "ArgumentError", "E6001", std::move(hint)) {}
...
public:
  NotImplementedError(const Position &pos_start, const Position &pos_end,
                      const string &details, shared_ptr<Context> context,
                      string hint = "")
      : RunTimeError(pos_start, pos_end, details, std::move(context),
                     "NotImplementedError", "E9004", std::move(hint)) {}
...
public:
  InvalidErrorTypeError(const Position &pos_start, const Position &pos_end,
                        const string &details, shared_ptr<Context> context,
                        string hint = "")
      : RunTimeError(
            pos_start, pos_end, details, std::move(context),
            "InvalidErrorTypeError", "E9003",
            hint.empty()
                ? "Valid error types are: RunTimeError, IllegalOperationError, "
                  "DivisionByZeroError, IndexOutOfBoundsError, NameError, "
                  "ArgumentError, TypeError, AttributeError, DictKeyError, "
                  "ValueError, ModuleError, StackDepthExceededError."
                : std::move(hint)) {}
...
public:
  DictKeyError(const Position &pos_start, const Position &pos_end,
               const string &details, shared_ptr<Context> context,
               string hint = "")
      : RunTimeError(pos_start, pos_end, details, std::move(context),
                     "DictKeyError", "E7003",
...
public:
  AttributeError(const Position &pos_start, const Position &pos_end,
                 const string &details, shared_ptr<Context> context,
                 string hint = "")
      : RunTimeError(pos_start, pos_end, details, std::move(context),
                     "AttributeError", "E7002", std::move(hint)) {}
...
public:
  TypeError(const Position &pos_start, const Position &pos_end,
            const string &details, shared_ptr<Context> context,
            string hint = "")
      : RunTimeError(pos_start, pos_end, details, std::move(context),
                     "TypeError", "E4001", std::move(hint)) {}
...
public:
  ValueError(const Position &pos_start, const Position &pos_end,
             const string &details, shared_ptr<Context> context,
             string hint = "")
      : RunTimeError(pos_start, pos_end, details, std::move(context),
                     "ValueError", "E9005", std::move(hint)) {}
...
public:
  ModuleError(const Position &pos_start, const Position &pos_end,
              const string &details, shared_ptr<Context> context,
              string hint = "")
      : RunTimeError(pos_start, pos_end, details, std::move(context),
                     "ModuleError", "E8001", std::move(hint)) {}
...
public:
  StackDepthExceededError(const Position &pos_start, const Position &pos_end,
                          const string &details, shared_ptr<Context> context,
                          string hint = "")
      : RunTimeError(
            pos_start, pos_end, details, std::move(context),
            "StackDepthExceededError", "E9002",
            hint.empty()
...
public:
  FileIOError(const Position &pos_start, const Position &pos_end,
              const string &details, shared_ptr<Context> context,
              string hint = "")
      : RunTimeError(pos_start, pos_end, details, std::move(context),
                     "FileIOError", "E9006",
                     hint.empty()
                         ? "Verify that the path is correct, the file exists, "
...
public:
  ...
  UserDefinedError(const Position &pos_start, const Position &pos_end,
                   shared_ptr<DataType> inst, shared_ptr<Context> context,
                   string hint = "");
